        ...
        try {
          ...
          if (isAdmin) {
            examQuery = supabase
              .from("exams")
              .select(selectFields)
              .eq("test_category", "Synthesis (non-SSQB)");
          } else {
            examQuery = supabase
          ...
          if (isAdmin) {
            ...
            exams = examData
              ? examData
                  ...
                  .filter(Boolean)
                  .filter(
                    (exam) => exam.test_category === "Synthesis (non-SSQB)"
                  )
              : [];
          }
          setUserExams(exams);
      ...
      {!isFetchingExams && !fetchError && filteredExams.length === 0 && (
        <p className="text-center text-gray-500 mt-6">
          {userExams.length === 0 && initialLoadComplete.current
            ? isAdmin
              ? "Không tìm thấy bài thi Synthesis nào trong hệ thống."
              : "Bạn hiện không có quyền truy cập vào bài thi Synthesis nào."
...
export default SynthesisExamsPage;
